...
/* PWM servo control - inverted from datasheet due to actual servo behavior */
#define PWM_RETRACTED_US 1900     /* fully extended position (inverted) */
#define PWM_EXTENDED_US  1100     /* fully retracted position (inverted) */
#define PWM_FAILSAFE_US  1500     /* failsafe mid position */
...
static rc_state_t rc = {
    .ch1 = FAILSAFE_CENTER, .ch2 = FAILSAFE_CENTER,
...
static void    update_servo_pwm(uint16_t ch1_value);
/* USER CODE END PFP */
...
/*
 * Map CH1 (0..2047) to servo PWM pulse width.
 * Observed behavior: 1900µs = extended, 1100µs = retracted
 * Desired: CH1 300 (low) = retracted, CH1 1600 (high) = extended
 * So: CH1 300 → 1100µs, CH1 1600 → 1900µs
 */
static void update_servo_pwm(uint16_t ch1_value) {
    extern TIM_HandleTypeDef htim2;  /* will be generated by CubeMX */

    uint32_t pulse_us;
    if (ch1_value < 300) {
        pulse_us = 1100;  /* clamp low → retracted */
    } else if (ch1_value > 1600) {
        pulse_us = 1900;  /* clamp high → extended */
    } else {
        /* Linear interpolation: higher ch1 → longer pulse_us */
        pulse_us = 1100 + ((ch1_value - 300) * (1900 - 1100)) / (1600 - 300);
    }

    /* Convert µs to timer counts: pulse_us * (PWM_MAX_CCR + 1) / PWM_PERIOD_US */
...
int main(void)
{
  ...
  update_servo_pwm(FAILSAFE_CENTER);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    if (rc.link_ok && (now - rc.last_frame_tick) > FAILSAFE_MS) {
        rc.link_ok = 0;
        rc.ch1 = rc.ch2 = rc.ch3 = rc.ch4 = FAILSAFE_CENTER;
        update_servo_pwm(FAILSAFE_CENTER);  /* move to failsafe position */
    }
