...
/* PWM servo control */
#define PWM_RETRACTED_US 1900     /* fully retracted position */
#define PWM_EXTENDED_US  1100     /* fully extended position */
 ...
 * CH1 ~300 (min) → 1900µs (fully retracted)
 * CH1 ~600 (failsafe) → 1500µs (mid position)
 * CH1 ~1600 (max) → 1100µs (fully extended)
 *
 * Linear mapping: higher CH1 = shorter pulse = more extended.
 */
static void update_servo_pwm(uint16_t ch1_value) {
    ...
    uint32_t pulse_us;
    if (ch1_value < 300) {
        pulse_us = PWM_RETRACTED_US;  /* clamp low */
    } else if (ch1_value > 1600) {
        pulse_us = PWM_EXTENDED_US;   /* clamp high */
    } else {
        /* Linear interpolation: higher ch1 → lower pulse_us */
        pulse_us = PWM_RETRACTED_US - ((ch1_value - 300) * (PWM_RETRACTED_US - PWM_EXTENDED_US)) / (1600 - 300);
    }
